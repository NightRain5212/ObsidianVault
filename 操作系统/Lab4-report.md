...
- 请简单说明proc_struct中各个成员变量含义以及作用。它与理论课中讲述的PCB结构有何不同之处？
...
- 与PCB相比多了`list_link`字段，进程环形链表用于管理进程，多了`hash_link`字段用于快速找到进程。进程内存用`mm_strcut`统一表示。`need_resched`字段用于进程调度。

# 2

- 试简单分析 uCore中 内核线程的创建过程。


- kernel_thread() 创建请求：do_fork
- 设置中断帧（trapframe）
- do_fork() 核心创建函数：alloc_proc，setup_kstack，copy_mm，copy_thread，wakeup_proc
- alloc_proc() 分配PCB
- setup_kstack() 分配内核栈
