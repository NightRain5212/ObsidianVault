...
- 用户进程实现了隔离与保护：
- 每个进程有自己的虚拟地址空间，进程无法直接访问内核或其他进程的内存。
- 使得我们能够在一个物理计算机上同时运行多个不信任的、可能包含错误的程序，而不会相互干扰或破坏系统。
...
- 所有系统调用通过统一入口`syscall()`函数，通过系统调用号直接索引跳转表，O(1)时间找到处理函数，参数从陷阱帧获取，用户态调用接口与内核态处理逻辑分离。基于跳转表的调用方式具有确定性和可预测性，而且便于扩展维护。具有隔离性质，有一定的安全性
