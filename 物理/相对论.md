...
- **时间的测量是相对的**。
- 由于$$\Delta t^{\prime}=\gamma(\Delta t-\frac\beta c\Delta x)$$
...
- 寻找适合光速不变原理的新的时空变换。
- 重要常量$$\gamma = \frac{1}{\sqrt{1-\frac{u^2}{c^2}}}$$
$$\beta = \frac{u}{c}$$
...
- 由洛伦兹变换,可以得到相对论速度变换$$\frac{\mathbf{d}x^{\prime}}{\mathbf{d}t}=\frac{\boldsymbol{\upsilon}_x-\boldsymbol{u}}{\sqrt{1-\frac{u^2}{c^2}}}$$
...
且$$\vec F = \frac{d\vec p}{dt} = \frac{d}{dt}(mv)$$
...
$$mv\operatorname{d}v=c^2\operatorname{d}m-v^2\operatorname{d}m$$所以$$\begin{cases}mv\operatorname{d}\boldsymbol{v}=c^2\operatorname{d}m-\boldsymbol{v}^2\operatorname{d}m\\dE_K=m\operatorname{v}\operatorname{d}v+v^2\operatorname{d}\boldsymbol{m}&\end{cases}$$$$\mathbf{d}E_k=c^2\operatorname{d}m$$
